...
- 그냥 mean(X)=평균추정량=분산추정량 이라고 주장하면 안되나? => 됩니다! 이게 바로 MLE에요!  (MLE=최대우도추정량) 
"""
...
function try_until_you_succeed(p) 
	n_of_try=0 
	u=0
...
let 
	...
	p=λ/n
	Δt = (1/n) # 단위가 시간1이니까 
	X = [try_until_you_succeed(p) for k in 1:N] .* Δt   #횟수*단위시간
	p1 = X|> histogram
...
# ╔═╡ 09a1a38c-2cf6-4b23-8b29-972aafd0c40e
# G(x) = 1-exp(-x/5)
# G: x -> *(-1/5) -> exp -> *(-1) -> +1
# Ginv: y -> -1 -> *(-1) -> log -> *(-5)
Ginv(x) = -5log(-(x-1))

# ╔═╡ b6d73c38-d13d-4034-a805-8f01b308545a
G(x) = 1-exp(-x/5)

# ╔═╡ aa69557c-c896-4336-bf50-174b8ec8a27b
Ginv(G(6))
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "7aa8e05c03cda68c393feaa1d52225fb46ce410d"
