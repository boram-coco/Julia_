...
let
	...
	p1 = plot(x -> -exp(-x)+1,0,20, title="평균=1")
	scatter!([0,0,0,0,0],u)   # x는 다 0인 유니폼 5개
	scatter!(Finv.(u),[0,0,0,0,0])  # y는 다 1인 유니폼 5개
	p2 = plot(x -> -exp(-x/5)+1,0,20, title="평균=5")
	scatter!([0,0,0,0,0],u)
...
let
	...
	lhs= length(X[X.>t])/N   # 좌변
	rhs= length(X[X.>t+s])/length(X[X.>s])   #우변
	md"""
	-  $t=$ $t
	-  $s=$ $s
	-  $P(X>t)=$ $(lhs)
...
let 
	N = 5000
	X = rand(Exponential(1),N)  #평균이 1인거 N개 뽑기
...
# ╔═╡ fa4a3bb5-2bd8-4b3f-abd6-60753b4bcdff
# 평균이 0이고 분산이 1인.. 
...
# 빨간선의 길이가 반지름. 길이를 제곱해서 2로 나누면 지수분포를 따름
...
# ╔═╡ 3e683e90-d3c5-4956-bc3b-82eefb1bed52
let
	p1= histogram(rand(10000) .|> x -> -log(1-x))
	N=10000
	X = rand(Normal(0,1),N)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "6b002999459f6ea8f733e0fea8f3473d31c8033c"
